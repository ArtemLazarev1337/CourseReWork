...
class FunctionalMenu : public BaseMenu {
    public:
        FunctionalMenu(const DynamicArray<std::string>& options) : BaseMenu(options) {}
        ...
        static void printTable(DynamicArray<Record>& records) {
        ...
        static void customSort(DynamicArray<Record>& records, bool (*compare)(const Record&, const Record&, bool ascending), bool ascending = true) {
            bool isSorted = false;
            while (!isSorted) {
                isSorted = true;
        ...
        friend void printListOfBrands(DynamicArray<Record>& records);

        friend void findByOwnerName(DynamicArray<Record>& records, const std::string& ownerSurname);
};
...
void printListOfBrands(DynamicArray<Record>& records) {
    ...
    unsigned int id = 1;
    std::cout << "┌────┬───────────────┬───────┐\n"
            << "│\e[1m" << FunctionalMenu::centerText("ID", colWidths[0]) << "\e[0m│\e[1m"
            << FunctionalMenu::centerText("Brand", colWidths[1]) << "\e[0m│\e[1m"
            << FunctionalMenu::centerText("Count", colWidths[2]) << "\e[0m│\n"
            << "├────┼───────────────┼───────┤\n";;
    for (std::pair<std::string, int> pair : brandCounts) {
        std::cout << "│\e[1m" << FunctionalMenu::centerText(std::to_string(id), colWidths[0]) << "\e[0m│"
                << FunctionalMenu::centerText(pair.first, colWidths[1]) << "│"
                << FunctionalMenu::centerText(std::to_string(pair.second), colWidths[2]) << "│\n";
...
void findByOwnerName(DynamicArray<Record>& records, const std::string& ownerSurname) {
    ...
    FunctionalMenu::customSort(result, &FunctionalMenu::compareByCarBrandAndOwnerName);
    FunctionalMenu::printTable(result);
};
